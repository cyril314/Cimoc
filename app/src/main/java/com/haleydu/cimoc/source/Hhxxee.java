...
public class Hhxxee extends MangaParser {
    ...
    public static final String HOST_STR = "997700.cc";
    ...
    public Request getSearchRequest(String keyword, int page) throws UnsupportedEncodingException {
        String url = "";
        if (page == 1)
            url = URL_STR + "/search/s.aspx";
        RequestBody requestBodyPost = new FormBody.Builder().add("tbSTxt", keyword).build();
        return HttpUtils.getMobileRequest(url).post(requestBodyPost).build();
    }
    ...
    public SearchIterator getSearchIterator(String html, int page) {
        ...
        return new NodeIterator(body.list(".cInfoItem")) {
            ...
            protected Comic parse(Node node) {
                String cid = node.href(".cListTitle > a").substring((URL_STR + "/comic/").length());
    ...
    public Request getInfoRequest(String cid) {
        String url = URL_STR + "/comic/".concat(cid);
    ...
    public Request getImagesRequest(String cid, String path) {
        String url = StringUtils.format(URL_STR + "/comic/%s/%s/", cid, path);
        return HttpUtils.getSimpleMobileRequest(url);
    }
