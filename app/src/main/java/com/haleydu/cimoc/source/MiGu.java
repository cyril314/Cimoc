...
public class MiGu extends MangaParser {
    ...
    public static final String HOST_STR = "migudm.cc";
    ...
    public Request getSearchRequest(String keyword, int page) throws UnsupportedEncodingException {
        ...
        String url = StringUtils.format(URL_STR + "/search/result/list.html?hintKey=%s&hintType=2&pageSize=30&pageNo=%d",
                keywordconv,
                page);
    ...
    public String getUrl(String cid) {
        return StringUtils.format(URL_STR + "/comic/%s.html", cid);
    }
    ...
    public Request getInfoRequest(String cid) {
        return new Request.Builder().url(StringUtils.format(URL_STR + "/comic/%s.html", cid)).build();
    }
...
//        String author = body.text("p.author").trim();
    ...
    public Request getImagesRequest(String cid, String path) {
        _cid = cid;
        _path = path;
        String url = StringUtils.format(URL_STR + "/%s/chapter/%s.html", cid, path);
        String html = httpGet(url);
        Matcher m = Pattern.compile("<input type=\"hidden\" id=\"playUrl\" value=\"(.*)\">").matcher(html);
        if (m.find()) {
            url = URL_STR + "/opus/webQueryWatchOpusInfo.html?".concat(m.group(1));
            return new Request.Builder().url(url).build();
        }
        return null;
    }
