...
public class ChapterManager {
    ...
    public Observable<List<Chapter>> getListChapter(Long sourceComic) {
        return mChapterDao.queryBuilder().where(Properties.SourceComic.eq(sourceComic)).rx().list();
    }

    public List<Chapter> getChapter(String path, String title) {
        return mChapterDao.queryBuilder().where(ChapterDao.Properties.Path.eq(path), ChapterDao.Properties.Title.eq(title)).list();
    }
