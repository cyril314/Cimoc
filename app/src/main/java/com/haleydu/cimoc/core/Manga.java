...
public class Manga {
    ...
    private static final String TAG = Manga.class.getSimpleName();
...
//                        if (comic != null && (comic.getTitle().indexOf(keyword) != -1 || comic.getAuthor().indexOf(keyword) != -1)) {
                        if (comic != null && (indexOfIgnoreCase(comic.getTitle(), keyword)
                                || indexOfIgnoreCase(comic.getAuthor(), keyword) || (!strictSearch))) {
...
//                        mongo.UpdateComicBase(comic, list);
    ...
    public static Observable<List<ImageUrl>> getChapterImage(final Chapter chapter, final Parser parser, final String cid, final String path) {
...
//              RequestBody body = new FormBody.Builder().add("key", keyword).add("s", "1").build();
                Request request = HttpUtils.getSimpleMobileRequest("http://m.ac.qq.com/search/smart?word=" + keyword);
                try {
